...
/**
 * @issues
 * https://github.com/PeachScript/vue-infinite-loading/issues/79
 */
  ...
  methods: {
    getCurrentDistance () {
      ...
      if (this.direction === 'top') {
        distance = isNaN(this.scrollParent.scrollTop) ? this.scrollParent.pageYOffset : this.scrollParent.scrollTop
      } else if (this.direction === 'left') {
        distance = isNaN(this.scrollParent.scrollLeft) ? this.scrollParent.pageXOffset : this.scrollParent.scrollLeft
        ...
        const scrollElmOffsetLeftFromRight =
          this.scrollParent === window ? window.innerWidth : this.scrollParent.getBoundingClientRect().right
