...
export default {
  ...
  mounted () {
    EventBus.$on('addNewLine', (lenght, item) => {
      ...
      if (item === 1) {
        this.AnalysisListIten1.data.push({ data: dataDefaultRow,
          row: lenght + 1})
      } else if (item === 2) {
        ...
        this.OtherTrend.push({ data: dataDefaultOtherTrend, item: 3 })
      } else {
        return false
      }
    })
    EventBus.$on('removeLine', (index, item) => {
      if (item === 1) {
        ...
        this.OtherTrend.splice(index, 1)
      } else {
        return false
      }
    })
